...
export default function Page() {
  ...
  const handleCheckout = () => {
    ...
    const productTitles = cart.map((item) => `${item.name} (x${item.quantity})`).join(", ")
    const totalPrice = getTotalPrice()

    // Usar el primer producto para obtener el restaurantId, o un valor por defecto
    const restaurantId = cart[0]?.restaurantId || "123"

    const params = new URLSearchParams({
      restaurantId: restaurantId,
      title: productTitles,
      price: totalPrice.toString(),
      quantity: "1", // Como es un resumen, la cantidad es 1
      total: totalPrice.toString(),
    })

    // Redirigir a la página de payment
    window.location.href = `/Pagos/payment?${params.toString()}`
  }
